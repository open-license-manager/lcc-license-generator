...
static void printHelp(const char *prog_name, const po::options_description &options) {
	...
	cout << prog_name << " [options] product_name1 product_name2 ... " << endl << endl;
...
static void initializeProject(const po::parsed_options &parsed, po::variables_map &vm, const char **argv,
	...
	project_desc.add_options()  //
		("name,n", po::value<std::string>(&project_name)->required(), "New project name (required).")  //
		("primary-key", po::value<boost::optional<std::string>>(&primary_key),
...
static void issueLicense(const po::parsed_options &parsed, po::variables_map &vm, const char **argv,
	...
	license_desc.add_options()  //
		...
		("client-signature,s", po::value<string>(),
		...
		(PARAM_LICENSE_NAME ",l", po::value<string>(&license_name)->required(),
		 "License name. May contain / "
		 "that will be interpreded as subfolders."
		 "The license name is usually the client's name "
		 "for which you're issuing the license")  //
		(PARAM_PRODUCT_NAME ",n", po::value<boost::optional<std::string>>(),
		 "Product name (in case it doesm't correspond with project name).")  //
		...
		("start-version,t", po::value<string>()->default_value("0", "All Versions"),
		 "Specify the first version of the software this license apply to.")  //
		("end-version,n", po::value<string>()->default_value("0", "All Versions"),  //
		 "Specify the last version of the software this license apply to.")  //
		("extra-data,x", po::value<string>(), "Specify extra data to be included into the license");
	("help", "Print this help.");  //
