...
nssv_DISABLE_MSVC_WARNINGS(4455 26481 26472)
	...
	public:
		...
		nssv_constexpr basic_string_view(const CharT* s, size_type count) : data_(s), size_(count) {}

		nssv_constexpr basic_string_view(const CharT* s) : data_(s), size_(Traits::length(s)) {}
		...
		nssv_constexpr size_type find_last_not_of(basic_string_view v, size_type pos = npos) const nssv_noexcept  // (1)
		{
			return empty() ? npos
						   : pos >= size() ? find_last_not_of(v, size() - 1)
										   : to_pos(std::find_if(const_reverse_iterator(cbegin() + pos + 1), crend(),
																 not_in_view(v)));
		}
	...
	public:
		...
		basic_string_view(const std::basic_string<CharT, Traits, Allocator>& s) nssv_noexcept : data_(s.data()),
																								size_(s.size()) {}
...
template <class CharT, class Traits, class Allocator>
basic_string_view<CharT, Traits> to_string_view(const std::basic_string<CharT, Traits, Allocator>& s) {
...
public:
	...
	void set_element_notation(ElementNotation notation) { m_impl->parser_config.notation = notation; }
	 ...
	 * include "<name>" syntax.
	 */
	void include_template(const std::string& name, const Template& tmpl) { m_impl->included_templates[name] = tmpl; }
};
