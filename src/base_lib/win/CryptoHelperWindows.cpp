	...
	****************************/
	...
	static vector<uint8_t> export_privateKey_blob(const BCRYPT_KEY_HANDLE& m_hTmpKey) {
		...
		if (!NT_SUCCESS(status = BCryptExportKey(
			m_hTmpKey, nullptr, LEGACY_RSAPRIVATE_BLOB, nullptr, 0, &dwBlobLen, 0))) {
			throw logic_error(
		...
		if (!NT_SUCCESS(status = BCryptExportKey(
			m_hTmpKey, NULL, LEGACY_RSAPRIVATE_BLOB, &result[0], dwBlobLen, &dwBlobLen, 0))) {
			throw logic_error(
...
*/
	...
	*************************/
	...
	CryptoHelperWindows::CryptoHelperWindows() :
		m_hSignAlg(openSignatureProvider()), m_hHashAlg(openHashProvider()) {
	}
	...
	void CryptoHelperWindows::loadPrivateKey(const std::string& privateKey) {
		...
		if (CryptStringToBinaryA(privateKey.c_str(), 0, CRYPT_STRING_BASE64HEADER,
			...
			if (CryptStringToBinaryA(privateKey.c_str(), 0,
				...
				if (CryptDecodeObjectEx(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
					PKCS_RSA_PRIVATE_KEY, pbBuffer, dwBufferLen, CRYPT_DECODE_ALLOC_FLAG, NULL, &pki, &pkiLen)) {
					if (m_hTmpKey != nullptr) {
						BCryptDestroyKey(m_hTmpKey);
						m_hTmpKey = nullptr;
					}
					status = BCryptImportKeyPair(m_hSignAlg, NULL,
						LEGACY_RSAPRIVATE_BLOB, &m_hTmpKey, (PUCHAR)pki, pkiLen, 0);
					if (NT_SUCCESS(status)) {
						LocalFree(pki);
						LocalFree(pbBuffer);
						return;
					}
					errors = formatError(status);
				}
			}
		}
		...
		cerr << "Failed to load BASE64 private key." << errors << endl;
	...
	static bool hashData(BCRYPT_HASH_HANDLE& hHash, const string& data,
		...
		bool success = false;
		if (NT_SUCCESS(
			status = BCryptHashData(hHash, (BYTE*)data.c_str(), (ULONG)data.length(),
				0))) {
			success = NT_SUCCESS(
		...
		if (NT_SUCCESS(
			...
			if (NULL != pbSignature) {
				if (NT_SUCCESS(
					...
					DWORD finalSize;
					if (CryptBinaryToString(pbSignature, cbSignature,
						CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, nullptr,
						&finalSize)) {
						signatureBuffer.resize(finalSize - 1); //finalSize counts the \0 in the end, while string counts only the characters
						success = CryptBinaryToString(pbSignature, cbSignature,
	...
	const string CryptoHelperWindows::signString(const string& license) const {
		...
		if (NT_SUCCESS(
			...
			(PBYTE)&cbHashObject, sizeof(DWORD), &cbData,
				0)) &&
			NT_SUCCESS(status = BCryptGetProperty(m_hHashAlg, BCRYPT_HASH_LENGTH,
			(PBYTE)&cbHashDataLenght, sizeof(DWORD), &cbData, 0))) {
			...
			if (NULL != pbHashObject && nullptr != pbHashData) {
				...
				if (NT_SUCCESS(
					...
					success = hashData(hHash, license, error, pbHashData,
						cbHashDataLenght)
